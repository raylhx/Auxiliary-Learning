<template>
  <div id='cet4'>
    <leftNav v-on:getMenuTag='ToGetList' requestType="cet4"></leftNav>
    <div class='container'>
          ...
          <ul class="note-list">
            <div class="nav-wrapper">
              ...
              <el-breadcrumb separator-class="el-icon-arrow-right">
                ...
                <el-breadcrumb-item>CET4</el-breadcrumb-item>
...
<script type='text/ecmascript-6'>
  ...
  export default{
    ...
    methods: {
      GetRandomWord () {
        Axios.get(URL + '/sys/vocabularies/randomWord').then((res) => {
          // 防止后台出现没有值返回的情况
      ...
      initGetList () {
        // 初始化获取列表，不涉及参数
        // 接口参数可能还有待调整
        Axios.get(URL + '/articles/cet4').then((res) => {
          let initResult = res.data
          if (initResult.length > 0) {
            this.NoListTooltip = false
            this.listAllData = initResult
            this.totalList = initResult.length
            for (let i = 0; i < initResult.length; i++) {
              initResult[i].router = HandelTitleString(initResult[i].title)
            }
            this.currentPage = 1
            this.handleCurrentChange(1)
          } else {
...
<style rel='stylesheet/stylus' lang='stylus'>
  /* @import '../../style/components/cet4.styl'; */
